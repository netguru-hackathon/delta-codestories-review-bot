defmodule CodestoriesReviewBotWeb.CategoryControllerTest do
  ...
  describe "execute/2" do
    ...
    test "adds reviewer with category", %{conn: conn} do
      ...
      assert "You've been added as Codestories reviewer to category 'RoR'" == text_response(conn, 200)
      assert %Reviewer{slack_id: "789"} = Repo.get_by(Reviewer, slack_id: "789")
    end

    test "removes reviewer from category", %{conn: conn} do
      conn = post(conn, Routes.commands_path(conn, :execute), %{text: "remove_from ror", user_id: "123"})
      assert "You're no longer Codestories reviewer for category 'RoR'" == text_response(conn, 200)
